...
export const Carousel = ({ children: slides }) => {
    ...
    <div className='overflow-hidden relative'>
        <div className='flex max-h-48 transition-transform ease-out duration-500' style={{transform: `translateX(-${curr * 100}%)`}}>
            {slides}
        </div>
        <div className='absolute top-4 w-full flex justify-center items-center'>
            <h1 className='text-2xl text-white font-bold'>Comentario en la página</h1>
        </div>
        <div className='absolute inset-0 flex items-center justify-between p-4'>
            <button onClick={prev} className='p-1 rounded-full shadow bg-white opacity-80 text-gray-800 hover:bg-white hover:opacity-90'>
            <ChevronLeft size={30} />
            </button>
